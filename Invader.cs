		...
		class Invader
		{
				...
				static void Update()
				{
						...
						for(int i=0;i<invader.Length;i++)
						{
								...
								if(invader[i].y>Console.WindowHeight-2 | invader[i].y<0 | invader[i].x>Console.WindowWidth-1 | invader[i].x<0)
								{
										invader[i].status=0;
								}
								if(invader[i].status==1)
								{
										...
										if(invader[i].isbomb)
										{
												...
												if(invader[i].c=="|" || invader[i].c=="/")
												{
														if(invader[i].life>1)
																Console.ForegroundColor=ConsoleColor.Magenta;
												}
												if(invader[i].c=="Y" || invader[i].c=="*")
												{
				...
				static void MoveInvaderBombs(int i)
				{
						...
						switch(invader[i].c)
						{
								case "/":
										...
										{
												invader[i].variant=new int [5];
												invader[i].variant[3]=invader[i].x;
												invader[i].variant[4]=invader[i].y;
												if(Math.Abs(invader[i].x-p.x)>Math.Abs(invader[i].y-p.y))
												{
														invader[i].variant[0]=0;//x++ or x--
														invader[i].variant[1]=100*(invader[i].y-p.y)/(invader[i].x-p.x);
														invader[i].variant[2]=1;//x++
														if(invader[i].x>p.x)
																invader[i].variant[2]=-1;//x--
												}
												else
												{
														invader[i].variant[0]=1;//y++ or y--
														invader[i].variant[1]=100*(invader[i].x-p.x)/(invader[i].y-p.y);
														invader[i].variant[2]=1;//y++
														if(invader[i].y>p.y)
																invader[i].variant[2]=-1;//y--
												}
										}
										...
										{
												if(invader[i].variant[0]==0)//x++ or x--
												{
														invader[i].x+=invader[i].variant[2];
														invader[i].y=invader[i].variant[4]+invader[i].variant[1]*invader[i].x/100;
												}
												else//y++ or y--
												{
														invader[i].y+=invader[i].variant[2];
														invader[i].x=invader[i].variant[3]+invader[i].variant[1]*invader[i].y/100;
												}
										}
										break;
								case "Y":
						...
						switch(invader[i].c)
						{
								...
								case "*":
										...
										if(invader[i].interval%8==1)
										{
												...
												invader[invader.Length-1].count=invader[i].count;
												invader[invader.Length-1].status=1;
												invader[invader.Length-1].life=1;
												invader[invader.Length-1].isbomb=true;
												invader[invader.Length-1].c="/";
												invader[invader.Length-1].name="*";
												Array.Resize(ref invader,invader.Length+1);
